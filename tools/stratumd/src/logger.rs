...
pub fn init() {
    let state_dir = if let Ok(xdg_state) = env::var("XDG_STATE_HOME") {
        ...
        PathBuf::from(home).join(".local").join("state").join("stratum")
    } else {
        PathBuf::from("/tmp/stratum")
    };

    if let Err(e) = fs::create_dir_all(&state_dir) {
        eprintln!("Failed to create log directory {}: {}", state_dir.display(), e);
        return;
    }
    ...
    // Leak the guard to keep it alive for the whole process life
    Box::leak(Box::new(_guard));

    tracing::info!("Logging initialized. Target directory: {}", state_dir.display());
}
